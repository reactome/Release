...
die "No maven3 installed, sorry I can't run" unless `which mvn3`;
...
# first, clear the path for the new index, if necessary
if (-d "$solr_dir/reactome_v$prev_release") {
    say STDERR "I am getting ready to index release $opt_r.  First I need to move $prev_release out of the way";
    chdir $solr_dir or die "Could not cd to $solr_dir $!";
    system "tar czvf reactome_v$prev_release.tar.gz reactome_v$prev_release";
    system "mv reactome_v$prev_release reactome_v$opt_r";
    system "chmod -R g+w reactome_v$opt_r";
    system "perl -i -pe 's/reactome_v$prev_release/reactome_v$opt_r/' reactome_v$opt_r/core.properties";
    chdir $present_dir or die "Could not cd to $present_dir $!";
}

# then index.  This actually uses tomcat/solr to write to the filesystem!
say STDERR "I am updating and compiling the indexer now...";
...
say STDERR "I am indexing now...";
system("java -jar -Xms5120M -Xmx10240M indexer.jar -d $opt_db -u $opt_user -p $opt_pass -s $solr_url ".
       "-e $solr_user -a $solr_pass -c src/main/resources/controlledvocabulary.csv -o $output -r $opt_r ");
system("gzip -f $output");

say "$0 has finished its job\n";
