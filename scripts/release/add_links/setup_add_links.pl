...
my ($addlinks_dir, $configuration, $rebuild_jar, $user, $pass, $host, $port, $db, $person_id);

# Parse commandline
my $usage = "Usage: $PROGRAM_NAME -addlinks_dir repo_path -configuration download_only|insertion_only|full " .
    "-rebuild_jar -user db_user -host db_host -pass db_pass -port db_port -db db_name -person_id person_db_id\n";
GetOptions(
...
($configuration && $configuration =~ /^download_only|insertion_only|full$/)
    || die "Need -configuration (download_only|insertion_only|full)\n";
...
sub update_addlinks_repo {
    my $addlinks_directory = shift;

    my $current_dir = getcwd;
    chdir $addlinks_directory;
    system 'git pull';
    chdir $current_dir;
...
sub clone_addlinks_repo {
    ...
    system "git clone https://github.com/reactome/add-links.git $addlinks_directory";
...
sub create_resources_symbolic_link {
    ...
    if (!(-e 'resources')) {
        system 'ln -sf ../src/main/resources resources';
    }
    chdir $current_dir;
...
sub configure_application_context {
    ...
    return if $configuration_option eq 'full';
    ...
    $file_contents =~ s/<!--.*?-->//gms;
    if ($configuration_option eq 'download_only') {
    ...
    } elsif ($configuration_option eq 'insertion_only') {
