...
#	       'BlackBoxEvent' => {'attributes' => [qw(hasEvent)]}, #ignore this for the time being
#	       'Regulation' => {'attributes' => [qw(regulator)]},
#	       'ReactionlikeEvent' => {'attributes' => [qw(input output catalystActivity hasMember)]},
...
#	       'ReactionlikeEvent' => {'attributes' => [qw(input output catalystActivity hasMember)]},
	       'ReactionlikeEvent' => {'attributes' => [qw(input output catalystActivity)]},
# Not sure if 'reverse_attributes' => [qw(hasEvent)] is appropriate here
	       'Event' => {'attributes' => [qw(goBiologicalProcess)], 'reverse_attributes' => [qw(hasEvent)]},
...
print "$0: $event_count events will be processed\n";
my $event_num = 0;
my $percent;
foreach my $r (@{$events}) {
    if ($event_num%100 == 0) {
       $percent = ($event_num * 100) / $event_count;
       print "$0: $event_num events ($percent %) have now been processed\n";
    }
    print $r->extended_displayName, "\n";
    my $cls = ($r->is_a('Pathway')) ? 'Pathway' : 'Reaction';
...
foreach my $p (@{$events}) {
	...
	$new_i->HasDirectComponent
	    (map{$to_cache->fetch($_) || die("No instance in dba_to cache for BD_ID $_")} map {$_->db_id} @{$p->HasEvent});
...
#	print "All components:\t", scalar(@{$all}), "\n";
	...
	my @leaves = grep {$_->is_a('Reaction') || ! $_->HasDirectComponent->[0]} @{$all};
...
#					  '_displayName' => $key
    ...
    if ($re->GeneName->[0]) {
	$participant->add_attribute_value_if_necessary('identifier',@{$re->GeneName});
    }
    $participant->add_attribute_value_if_necessary('identifier',@{$re->OtherIdentifier},$re->db_id);
    $participant->add_attribute_value_if_necessary('identifier',@{strip_version_number($participant->Identifier)});
