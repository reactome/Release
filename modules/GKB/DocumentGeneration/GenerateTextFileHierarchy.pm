...
sub open_stream {
    ...
    $logger->info("output to stream is not available for this text generator\n");
}
...
sub open_filename {
    ...
    $directory =~ s/\.txt$//;
    if (!(-e $directory)) {
        $logger->info("making directory=$directory\n");
        mkdir($directory);
    }
...
sub clean_up {
    ...
    $logger->info("entered, directory=$directory\n");

    if (opendir (DIR, $directory)) {
        my $file_counter = 0;
        $logger->info("about to do a readdir, directory=$directory\n");
...
#        while (my $file = readdir(DIR)) {
        ...
        $logger->error("could not open directory=$directory\n");
    }
...
sub generate_header {
    ...
    $logger->info("about to mkdir file_path=$file_path\n");

    mkdir($file_path);

    $logger->info("depth=$depth, current_section_heading_depth=$current_section_heading_depth, text=$text\n");
    ...
    if (open($current_filehandle, ">$file_name")) {
    	...
    	$logger->error("cannot create file handle for: $file_name\n");
...
sub generate_paragraph {
	...
	$logger->warn("filehandle is undef for text=$text, Im outta here!\n");
	return;
    }
