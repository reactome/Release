package GKB::Graphics::SimpleDraw::CoefficientAwareReaction;
use strict;
use GD;
...
sub draw {
  ...
  my @input_b       = $box_formatter->format_boxes_with_coefficient(@{$self->input});
  my $title_b       = $box_formatter->format_boxes($self->title);
  my @output_b      = $box_formatter->format_boxes_with_coefficient(@{$self->output});
  ...
  # create image
  my $gd  = GD::Image->new($max_width+3,$max_height+3,1) or croak;
  $gd->saveAlpha(0);
  $gd->alphaBlending(1);
  ...
  for (my $i=0;$i<@input_b;$i++) {
    # box
    $input_b[$i]->render_with_coefficient($gd,2+$right-$input_b[$i]->width,$top,{box=>1,fill=>INPUT_COLOR});
    # arrow
    my $x1 = $right+3;
  ...
  for (my $i=0;$i<@output_b;$i++) {
    # box
    $output_b[$i]->render_with_coefficient($gd,$left,$top,{box=>1,fill=>OUTPUT_COLOR});
    # arrow
  ...
  $gd->rectangle(0,$reaction_top,$gd->width-1,$gd->height-$extra_height-2,$gd->colorResolve(0,0,0));
