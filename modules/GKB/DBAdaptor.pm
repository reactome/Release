...
#    print join("\n", map {defined $_ ? $_ : 'undefined'} @args), "\n";
    ...
    my $dbh = eval { get_secure_connection($dsn, $user, $password) };

    # Try normal connection if secure connection isn't accepted
    if ($@ || !$dbh) {
        $dbh = eval { get_normal_connection($dsn, $user, $password) };
...
sub get_secure_connection {
    my $dsn = shift;
    my $user = shift;
    my $password = shift;

    return get_connection($dsn, $user, $password, 1);
}

sub get_normal_connection {
    my $dsn = shift;
    my $user = shift;
    my $password = shift;

    return get_connection($dsn, $user, $password, 0);
}
...
sub get_connection_error {
    ...
    my $db = $connection_parameters->{'db'};
    my $user = $connection_parameters->{'user'};
    my $dsn = $connection_parameters->{'dsn'};
