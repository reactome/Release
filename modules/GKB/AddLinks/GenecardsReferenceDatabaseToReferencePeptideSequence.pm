...
GKB::AddLinks::GeneCardsReferenceDatabaseToReferencePeptideSequence
...
package GKB::AddLinks::GenecardsReferenceDatabaseToReferencePeptideSequence;
...
sub buildPart {
	...
	print STDERR "\n\nGeneCardsReferenceDatabaseToReferencePeptideSequence.buildPart: entered\n";
	...
	$self->set_instance_edit_note("${attribute}s inserted by GeneCardsReferenceDatabaseToReferencePeptideSequence");
	...
	my $genecards_reference_database = $self->builder_params->reference_database->get_genecards_reference_database();
	foreach my $reference_peptide_sequence (@{$reference_peptide_sequences}) {
		...
		print STDERR "GeneCardsReferenceDatabaseToReferencePeptideSequence.buildPart: i->Identifier=" . $reference_peptide_sequence->Identifier->[0] . "\n";
		
		# Remove Genecards identifiers to make sure the mapping is up-to-date, keep others
	    ...
	    $self->remove_typed_instances_from_attribute($reference_peptide_sequence, $attribute, $genecards_reference_database);
	    
	    # We are expecting a 1 to 1 mapping between UniProt and
	    # Genecards identifiers.
	    my $genecards_database_identifier = $self->builder_params->database_identifier->get_genecards_database_identifier($reference_peptide_sequence->Identifier->[0]);
	    $reference_peptide_sequence->add_attribute_value($attribute, $genecards_database_identifier);
