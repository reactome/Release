...
GKB::AddLinks::ZincMolecules

=head1 SYNOPSIS

=head1 DESCRIPTION

Adds ZINC linkers to ReferenceMolecules having chebi-IDs
...
sub buildPart {
    ...
    # A hash where the keys are ZINC Ids and the values are GKInstances
	...
	for my $molecule (@$molecules) { 
	    ...
	    ## Careful! ReferenceMolecules can be associated with > 1 ZINC ID.
		...
		->get_zinc_database_identifier( $zinc );
	    $molecule->add_attribute_value( $attribute, $zinc_database_identifier );
