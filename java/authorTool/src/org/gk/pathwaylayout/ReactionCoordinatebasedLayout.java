 ...
 * @author vastrik
 *
 * This class was originally written for creating entity level diagrams of the whole sky.
 */
public class ReactionCoordinatebasedLayout implements Runnable {
...
//				(int)reactionMidPoint.getY()*PathwayLayoutConstants.COORDINATE_SCALING_FACTOR);
	...
	public Collection getReactions() throws Exception {
		Collection reactions = Utils.getLocatedReactionsForSpecies(dba,this.focusSpecies);
...
//			}
	...
	/*
	 * Insert all output entities which haven't been inserted already.
	 */
...
//				continue;
	...
	public void setReactionOutputInserted(GKInstance reaction, GKInstance entity) {
	...
	public boolean isReactionOutputInserted(GKInstance reaction, GKInstance entity) {
	...
	public Edge createEdge(Vertex from, Vertex to, int edgeType) {
		...
		GraphConstants.setLineWidth(edge.getAttributes(), PathwayLayoutConstants.EDGE_THICKNESS);
		edges.add(edge);
		return edge;
	}
	...
	public Edge createCatalystEdge(Vertex from, Vertex to) {
		...
		GraphConstants.setDashPattern(edge.getAttributes(), PathwayLayoutConstants.CATALYST_EDGE_PATTERN);
		return edge;
	}
	...
	public DefaultEdge createRegulatorEdge(Vertex from, Vertex to, String regulation) {
		...
		GraphConstants.setDashPattern(edge.getAttributes(), PathwayLayoutConstants.REGULATION_EDGE_PATTERN);
		return edge;
	}
...
//				}
	...
	synchronized void relax() {
		...
		for (Iterator ei = edges.iterator(); ei.hasNext();) {
			...
			Dimension dimension = Utils.smallestDistance(e.getTargetVertex().getBounds(), e.getSourceVertex().getBounds());
		...
		for (int i = 0 ; i < verteces.size() ; i++) {
			...
			for (int j = i + 1 ; j < verteces.size() ; j++) {
				...
				Dimension d = Utils.smallestDistance(n1.getBounds(), n2.getBounds());
	...
	synchronized void relaxNeighbourhood() {
		...
		for (Iterator ei = edges.iterator(); ei.hasNext();) {
			...
			Dimension dimension = Utils.smallestDistance(target.getBounds(), source.getBounds());
			double vx = dimension.getWidth();
			double vy = dimension.getHeight();
			double len = Math.sqrt(vx * vx + vy * vy);
			len = (len == 0) ? .0001 : len;
...
//			out.close();
	...
	public void saveAsTiles() throws IOException {
		...
		File topDir = new File("tiles" + "/" + this.dba.getDBName() + "/" + this.focusSpecies.getDBID());
		if (! topDir.exists())
			topDir.mkdirs();
	...
	public void saveZoomLevelAsTiles(double zoomLevel, File dir) throws IOException {
		...
		try { Thread.sleep(1000); } catch (InterruptedException e) { e.printStackTrace();}
		//Rectangle2D bounds = graph.getCellBounds(graph.getRoots());
		//int w = (int) ((10 + bounds.getWidth() + bounds.getX()) * zoomLevel);
		//int h = (int) ((10 + bounds.getHeight() + bounds.getY()) * zoomLevel);
...
/*				for (int xs = 0; xs*tileWitdh < trueTileW; xs++) {
	...
	public void placeEntitiesWithSingleEdge() {
		...
		for (Vertex r : reactionInstance2NodeMap.values()) {
			...
			if (!looseCatalyst.isEmpty()) {
				...
				PolarCoordinates pc = new PolarCoordinates(looseCatalyst.iterator().next().distanceFromCentreToBoundaryAtBearing(phi) + 3*PathwayLayoutConstants.REACTION_NODE_DIAMETER, phi);
				nested.putAll(spreadSatelliteNodesAroundPoleNode(r, pc, looseCatalyst, fixedCoordinates));
			}
			if (!looseRegulator.isEmpty()) {
				...
				if (fixedInput.isEmpty() && looseInput.isEmpty()) {
					...
					pc.r = looseRegulator.iterator().next().distanceFromCentreToBoundaryAtBearing(pc.phi) + 3*PathwayLayoutConstants.REACTION_NODE_DIAMETER;
				}
	...
	public void collapseClusteredCatalystVerteces () {
		...
		Map<GKInstance,List<Vertex>> m = Utils.getInstance2VertexMap(verteces);
		for (List<Vertex> l : m.values()) {
			if (l.size() == 1) continue;
			List<Vertex> catalysts = new ArrayList<Vertex>();
			for (Vertex v : l) {
				if (v.isCatalystOnly()) catalysts.add(v);
			}
			if (catalysts.size() < 2) continue;
			List<List<Vertex>> clusters = vc.cluster(catalysts);
			if (clusters == null) continue;
			for (List<Vertex> cluster : clusters) {
				collapseVerteces(cluster);
			}
		}
	...
	public void collapseVerteces1 (List<Vertex> vertexList) {
		...
		// Remove possible duplicate, i.e. w/ same source and target, edges from the Vertex
		List<Edge> duplicateEdges = v1.removeDuplicateEdges();
		edges.removeAll(duplicateEdges);
		model.remove(duplicateEdges.toArray());
		verteces.removeAll(remove);
		model.remove(remove.toArray());
		Map atts = new HashMap();
		GraphConstants.setBackground(atts, Color.red);
		Map m = new HashMap();
		m.put(v1, atts);
	...
	public void collapseVerteces (List<Vertex> vertexList) {
		...
		if (vertexList.size() == 2) {
			v1 = vertexList.get(0);
			Rectangle b1 = v1.getBounds();
			Rectangle b2 = vertexList.get(1).getBounds();
			b1.x += (b2.x - b1.x)/2;
			b1.y += (b2.y - b1.y)/2;
			Map atts = new HashMap();
			GraphConstants.setBounds(atts, b1);
			Map m = new HashMap();
			m.put(v1, atts);
			model.edit(m, null, null, null);
		} else {
			v1 = Utils.vertexClosestToMeanCentrePoint(vertexList);
		}
	...
	private GKInstance createInstance () throws InvalidAttributeException, Exception {
		...
		GKInstance i = new GKInstance(schema.getClassByName(ReactomeJavaConstants.PathwayDiagram));
		i.setAttributeValue(ReactomeJavaConstants.width, (int)(bounds.width * PathwayLayoutConstants.ZOOM_LEVELS[0]));
		i.setAttributeValue(ReactomeJavaConstants.height, (int)(bounds.height * PathwayLayoutConstants.ZOOM_LEVELS[0]));
	...
	public Point findBottomRight() {
		...
		for (int i = 0; i < verteces.size(); i++) {
			...
			max_x = Math.max(max_x, n.getX() + bounds.width);
			max_y = Math.max(max_y, n.getY() + bounds.height);
		}
